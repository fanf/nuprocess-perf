...
object RunHooks {
  // hook own threadpool
  implicit val executor = monix.execution.Scheduler.io(
    "rudder-hooks-exec",
  ...
  def asyncRun(
    ...
    hooks.hooksFile.foldLeft(Future(Ok("", ""): HookReturnCode)) {
      case (previousFuture, nextHookName) =>
            ...
            RunNuCommand.run(Cmd(path, Nil, env.toMap)).map { result =>
              ...
              case ex: Exception =>
                HookReturnCode.SystemError(
                  s"Exception when executing '${path}' with environment variables: ${env.show}: ${ex.getMessage}"
                )
            }
  ...
  def syncRun(
      ...
      hookParameters: HookEnvPairs,
      envVariables: HookEnvPairs,
      warnAfterMillis: Long = Long.MaxValue,
      killAfter: Duration = Duration.Inf
  ): HookReturnCode = {
    try {
      ...
      val res =
        Await.result(asyncRun(hooks, hookParameters, envVariables), killAfter)
      val duration = System.currentTimeMillis - time_0
      if (duration > warnAfterMillis) {
        HooksLogger.LongExecLogger.warn(
          s"Hook '${cmdInfo}' took more than configured expected max duration: ${duration} ms"
        )
      }
      ...
      case NonFatal(ex) =>
        HookReturnCode.SystemError(
          s"Error when executing hooks in directory '${hooks.basePath}'. Error message is: ${ex.getMessage}"
        )
    }
  }
}
