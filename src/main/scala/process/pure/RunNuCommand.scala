...
import process.pure.errors.IOResult
...
object RunNuCommand {
  ...
  private[this] class CmdProcessHandler(promise: Promise[Nothing, CmdResult])
    ...
    override def onExit(exitCode: Int): Unit = {
      promise
        .succeed(CmdResult(exitCode, stdout.toString, stderr.toString))
        .runNow
    }
  ...
  def run(
      cmd: Cmd,
      limit: Duration = 30.minute
  ): IOResult[Promise[Nothing, CmdResult]] = {
    /*
    ...
    def effect[A](effect: => A) =
      IOResult.effect(s"Error when executing command ${cmdInfo}")(effect)
    def effectNonBlocking[A](effect: => A) =
      IOResult.effectNonBlocking(s"Error when executing command ${cmdInfo}")(
        effect
      )
    ...
    for {
      ...
      ) // not effect: only instanciation of objects
