...
object Main {
  ...
  def compareRun(hooks: Hooks, iterations: Int): Unit = {
    def run(msg: String, params: Seq[HookEnvPairs],  env: HookEnvPairs, syncRun: (Hooks, HookEnvPairs, HookEnvPairs) => HookReturnCode): Unit = {
        val t0 = System.currentTimeMillis()
        params.foreach { p => syncRun(hooks, p, env) }
